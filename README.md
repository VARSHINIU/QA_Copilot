...
4. **Writing automation code** — converting a list of test cases into working Automation test scripts, by hand
5. **Updating test cases and automation code every time requirements change** — going back into existing test cases and Playwright scripts to add, modify, or remove steps as the feature evolves, instead of rewriting everything from scratch each time
...
│   │   ├── TestGenerator_Workflow.png       # shows the Test Generator and additional Requirements workflow
...
- Add a self-healing locator agent that suggests robust alternatives for
  broken Playwright selectors when the Test Generator's
